...
The aggregated time-series function delivers a single geospatial layer, where the time dimension is aggregated to one value per pixel, either a single value, count, index, or otherwise. It summarizes the percentage of per-pixel observations where the queried semi-concept(s) were detected based on the total number of "clean" pixels. In this case, "clean" refers to the exclusion of cloud-like and unknown semi-concepts, as well as pixels with a measured value of 0 in any of the 6 bands required for \acs{SIAM}™, which are identified using the no-data mask generated during pre-processing (*see* \autoref{sec:format_siam}). An example for one pixel being queried for vegetation-like semi-concepts is illustrated in \autoref{fig:veg_index_diagram}.
...
\begin{figure}
\centering
\includegraphics[width=0.9\textheight,keepaspectratio]{source/figures/maps/3yr_valid_total.png}
...
\begin{figure}
\centering
\includegraphics[width=0.9\textheight,keepaspectratio]{source/figures/maps/3yr_clean_discrete.png}
...
\begin{figure}
\centering
\includegraphics[width=0.9\textheight,keepaspectratio]{source/figures/maps/3yr_veg_total.png}
...
\begin{figure}
\centering
\includegraphics[width=0.9\textheight,keepaspectratio]{source/figures/maps/3yr_veg_norm.png}
...
\begin{figure}
\centering
\includegraphics[width=0.9\textheight,keepaspectratio]{source/figures/maps/3yr_water_total.png}
...
\begin{figure}
\centering
\includegraphics[width=0.9\textheight,keepaspectratio]{source/figures/maps/3yr_water_norm.png}
...
![(a): the spatial extent of the query; (b, c and d): the number of acquisitions available from 1 May to June 15 in each of the years. 2018 has considerably more acquisitions because Sentinel-2B became operational in the months following June 2017.  \label{fig:afrin_valid_total}](source/figures/maps/afrin_valid_total.png)

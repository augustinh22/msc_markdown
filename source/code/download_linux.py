...
#              images from the Sentinel API or Open Access Data Hub on Linux.
#
# Author:      h.Augustin
#
# Created:     07.06.2017
...
def get_args():
    ...
    if len(sys.argv) == 1:
        ...
        parser.add_argument(
                ...
                default='./tempS2')
        parser.add_argument(
                '-r', dest='MaxRecords', action='store', type=int,
                help='Maximum number of records to download (default=100)',
                default=100)
        parser.add_argument(
                '--hub', dest='hub', action='store',
        ...
        parser.add_argument(
                '-o', '--orbit', dest='rel_orbit', action='store',
                type=int, help='Relative orbit path number', default=None)
        parser.add_argument(
                '-od', '--orbit_dir', dest='orbit_dir', action='store',
                type=str, help='Orbit direction (e.g. asc or desc)',
                default=None)
        parser.add_argument(
                '-ord', '--orderby', dest='orderby', action='store',type=str,
                help=('Order results by ingestion (id) or capture date (d), '
                'asc or desc (e.g. \'id-asc\' for ingestion date, ascending)'),
                default=None)
        #
        # Sentinel-1 specific parameters.
        #
        parser.add_argument(
                '-s1pr', '--s1product', dest='s1product', action='store',
                type=str, help='Sentinel-1 product (e.g. SLC, GRD, OCN)',
                default=None)
        parser.add_argument(
                '-s1po', '--s1polar', dest='s1polar', action='store', type=str,
                help='Sentinel-1 polarisation mode (e.g. HH, VH, VV VH)',
                default=None)
        parser.add_argument(
                '-s1mo', '--s1mode', dest='s1mode', action='store', type=str,
                help='Sentinel-1 sensor operational mode (e.g. SM, IW, EW)',
                default=None)
        #
        # Sentinel-2 specific parameters.
        #
        parser.add_argument(
                '-c', '--max_cloud', dest='max_cloud', action='store',
                type=float, help='Only search for products up to a certain '
                'cloud percentage (e.g. 50 for 50 percent)', default=None)
        parser.add_argument(
                '-s2pr', '--s2product', dest='s2product', action='store',
...
def create_query():
    ...
    if options.tile is None or options.tile == '?':
        ...
        #
        # Defines lat+lon based on tile's center point using API or kml file.
        #
    ...
    # Beware of the quotation marks. For some reason double quotes alone are
...
def get_query_xml():
    ...
    #
    # Execute command to download and save query as xml-file in
    ...
    for entry in range(len(entries)):
        #
        # The UUID element is the key for creating the path to the file.
        #
...
def return_tiles(uuid_element, filename, tile=''):
    ...
    for granule_entry in range(len(granule_entries)):
        ...
        #
        # If one tile is given as optional arg, return entire tile file name.
        #
        if tile != '':
        ...
        try:
            with zipfile.ZipFile(zipped_path) as z:
                ...
                #
                    ...
                    print ('{} has already been successfully downloaded!'
                        ).format(zfile)
...
def download_results(entries):
    ...
    #
    # If you want to download all entries and did not search for a
    # specific tile, then downloading will begin.
    #
    if (options.tile is None or options.tile == '?'):
        #
        ...
        for entry in range(len(entries)):
            ...
            if check is True:
                ...
                try:
                    ...
                    continue
                #
                # If the unzipped and zipped version exist,
    ...
    # create the proper file structure mimicing a complete download and fill
    # it with data specific to the tile you want, or, post 06.12.16,
    # simply downloadcomplete matching tile packages.
    #
        ...
        for entry in range(len(entries)):
            ...
            if filename.startswith('S2A_OPER_'):
                #
                # Find tiles in entry, returning number[0] and tile names[1]
                #
            ...
            # File structure------------------------------------------------
                ...
                #
                # Create tile directory in GRANULE based on tile file name.
                #
            ...
            # Downloads--------------------------------------------------
                ...
                #
                # Download tile xml file and create AUX_DATA, IMG_DATA and
                # QI_DATA folders in tile folder and download their contents.
                #
                ...
                if check is True:
                    ...
                    # Save to defined directory (default = ./tempS2)
                    #
                    command_wget = '{} {} {}{}/{} "{}"'.format(wg, auth,
                         wg_opt, options.write_dir, zfile, sentinel_link)
                    #
                    # Execute download.
                    #
                    try:

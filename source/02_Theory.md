...
The \ac{UN}'s 193 Member States have identified and agreed to work towards 17 interconnected goals with many targets and a related indicator framework for the 2030 Agenda for Sustainable Development [@Resolution70Transforming2015]. These goals are known as the \acp{SDG} (Fig. \ref{fig:SDGs}), and replace what were the eight \acp{MDG}, presented by the \ac{UN} to be achieved by 2015. Development is a charged concept (e.g. what does it mean to be "developed" as a country?), with many different connotations. That is beyond the scope of this work and will not be discussed, but any reference to development refers to how it is characterised through the \acp{SDG}.
...
[^2]: Still need to incorporate <http://eohandbook.com/sdg/files/CEOS\_EOHB\_2018\_SDG.pdf>, @un-habitatSDGGoal112017 and http://eo4sdg.org/wp-content/uploads/2017/10/Primer\_ZeroHunger.pdf
...
Recent developments are the EarthServer (Baumann et al. 2015) based on a Rasdaman database, the Australian Geoscience Data Cube (Purss et al. 2015), which is currently implemented in a high performance computing (HPC) environment (Evans et al. 2015) or EarthDB (Planthaber, Stonebraker, and Frew 2012) based on a SciDB database (Stonebraker et al. 2013). A combination of fully automatic semantic enrichment of EO data and time series analysis in 3D data cubes are presented by Tiede et al. (2016). [RD8-16].
...
Incorporating all of the Sentinel-2 data available for an area including information layers generated using pre-classification in an implementation of the \ac{ODC} can be considered as providing the data in an analysis-ready way. Due to the fully automated preparation of data from acquisition to \ac{ODC} ingestion, this implementation can be considered highly repeatable. Reproducibility was a strong driver behind this implementation. The reproducibility of information extraction in this case is highly linked to the level of automation of information production, which is quite high. The methods and results ought to be highly reproducible, given access to the same Sentinel-2 data, versions of \ac{SIAM} and \ac{ODC} software, Python computing environments, code and queries.

The thesis being presented here utilises similar automated data preparation as @tiedeAutomaticPostclassificationLand2014 and @sudmannsAutomaticExpostFlood2017, including the prior-knowledge-based classification, but transfers it to Sentinel-2 imagery in an implementation of the \ac{ODC}, inspired by analysis capabilities of \ac{IQ}, the prototypical implementation described by @tiedeArchitecturePrototypicalImplementation2017 and the rest of the work conducted within the SemEO project.

The implementation leads towards supporting interactive, ad-hoc analysis where users are not required to download any data, rather only the results they would like to keep. Validation of the information generated by this implementation is not part of this thesis, though comparison to some existing information is made. It is intended to be viewed as a proof-of-concept and a spring-board for further research leading towards more time-sensitive analysis as well as monitoring long-term goals like the \acp{SDG}.

Due to the generic nature of the semantic enrichment implemented here, information can be generated for many purposes. The intent is to be able to provide \ac{EO}-based information that can help serve the following \acp{SDG}, targets and indicators [@Resolution70Transforming2015]. Two of the indicators (2.4.1 and 6.6.1) are considered tier 3, in that there is no internationally agreed methodology that exists for calculating them [@committeeonearthobservationsatellitesSatelliteEarthObservations2018]. The last indicator (15.1.1) is considered tier 1, in that established and acceptable methodology exists and data are already widely available:
